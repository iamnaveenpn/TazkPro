...
class Task(models.Model):
    ...
    # New date fields
    start_date = models.DateField(null=True, blank=True, help_text="Planned start date for the task")
    end_date = models.DateField(null=True, blank=True, help_text="Planned end date for the task")
    
    # Time tracking
    total_time = models.DurationField(null=True, blank=True, help_text="Total tracked time spent on task")
    ...
    def update_total_time(self):
        """Update total_time from all time tracking sessions"""
        from django.db.models import Sum
        total = self.time_tracking.filter(end_time__isnull=False).aggregate(
            total=Sum('duration')
        )['total']
        if total:
    ...
    def has_active_timer(self):
        ...
        return self.time_tracking.filter(is_active=True).exists()

    def get_active_timer(self):
        """Get the active timer for this task"""
        return self.time_tracking.filter(is_active=True).first()
...
class Notification(models.Model):
    ...
    request_data = models.JSONField(default=dict, blank=True, null=True, help_text="Additional data for requests")
...
class TimeTracking(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='time_tracking_sessions')
